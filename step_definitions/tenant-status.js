const url = 'build/resources/index.html';

given('Server tenant is running', () => {
    cy.server();
    cy.route('GET', 'build/API/system/tenant/*', 'fixture:tenantRunning').as('tenant');
});

given('Server tenant is paused', () => {
    cy.server();
    cy.route('GET', 'build/API/system/tenant/*', 'fixture:tenantPaused').as('tenant');
});

given('I\'m logged as technical user', () => {
    cy.server();
    cy.route('GET', 'build/API/system/session/unusedId', 'fixture:technicalUser').as('session');
});
